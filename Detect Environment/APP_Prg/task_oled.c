...
void task_oled(void *params)
{
	...
	while(1)
	{
		...
		{
			OLED_ShowString(0, 0, "待机模式", OLED_8X16);
		}
		...
		else if(System_Mode == SysMODE_SET)
		{
			...
			if(ChangeModelu == 0)
			{
				...
				OLED_ShowString(0, 0, "设置模式", OLED_8X16);
				OLED_ShowString(0, 16, "温度阈值设置", OLED_8X16);
				OLED_ShowString(0, 32, "音量设置", OLED_8X16);
				OLED_ShowString(0, 48, "ESP01S设置", OLED_8X16);
			...
			else if(ChangeModelu == 1)
			{
				...
				if(select_module_y == Second_Line)
				{
					...
					OLED_ShowString(0, 0, "温度阈值设置", OLED_8X16);
				...
				else if(select_module_y == Third_Line)
				{
					...
					OLED_ShowString(0, 0, "音量设置", OLED_8X16);
					OLED_Printf(16, 16, OLED_8X16, "按键音量:%d",volume[1]);
					OLED_Printf(16, 32, OLED_8X16, "警报音量:%d",volume[2]);
				...
				else if(select_module_y == Forth_Line)
				{
					...
					{
						OLED_ShowString(0, 24, "正在重连...", OLED_8X16);
						OLED_Update();
						esp01s_flag = esp01s_Init();
						OLED_Clear();
					}
					
					OLED_ShowString(0, 0, "ESP01S设置", OLED_8X16);
					OLED_ShowString(0, 16, "是否连接:", OLED_8X16);
					if(esp01s_flag)
					{
						OLED_ShowString(72, 16, "是", OLED_8X16);
					}
					else
					{
						OLED_ShowString(72, 16, "否", OLED_8X16);
					}
					OLED_ShowString(0, 32, "重新连接?", OLED_8X16);
