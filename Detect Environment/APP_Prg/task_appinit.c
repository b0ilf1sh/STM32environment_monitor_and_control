...
void app_init(void)
{
	...
	// OLED_Clear();
	// OLED_ShowString(0, 0, "CONNECT BROKER", OLED_8X16);
	// OLED_Update();
	
	// HAL_NVIC_SetPriority(TIM4_IRQn, 4, 0);//中断优先级在0到4不受freertos管理
	// HAL_NVIC_SetPriority(USART2_IRQn, 4, 0);//中断优先级在0到4不受freertos管理
	// MQTT_CONNECT_FLAG = MQTT_Init();
	// OLED_Clear();
	// if(MQTT_CONNECT_FLAG==0)
	// {
	// 	OLED_ShowString(0, 0, "CONNECT OK", OLED_8X16);
	// }
	// else
	// {
	// 	OLED_ShowString(0, 0, "CONNECT ERR", OLED_8X16);
	// }
	// OLED_Update();
